...
%%%%%%%%% NOTE: THIS SCRIPT IS NEARLY IDENTICAL TO qbit_simulate_master.m
%%%%%%%%% BUT NOW INCLUDES FLAP FORCES
...
Iyy = 2.32e-3;
span = 15*in2m;
l_P = 6*in2m;     % Distance between rotors
l_E = 5*in2m;   % Distance from flap aero center to center of mass
chord = 5*in2m;
R = 2.5*in2m;
...
%     Vw(i-1) = 1.2*sqrt( T_avg/(8*rho*pi*R^2) );
    Vw(i-1) = sqrt( (Vi(i-1)*cos(phi(i-1)-gamma(i-1)))^2 + (abs(T_avg)/(0.5*rho*pi*R^2)) );
...
%         Cm = 0.5*sin(3*alpha_e(i-1))*cos(3*alpha_e(i-1));
        ...
        Cd = 0;
        Cm = 0;
        alpha_e(i-1) = alpha(i-1);  % The angle of attack is now true.
    end
    ...
    [T_top(i), T_bot(i), F_E(i), Fdes(:,i)] = qbit_controller_flaps(current_state, desired_state(:,i), L(i-1), D(i-1), M_air(i-1), alpha_e(i-1), m, Iyy, l_P, l_E);
    
    xdotdot(i) = ((T_top(i) + T_bot(i))*cos(phi(i-1)) - F_E(i)*sin(phi(i-1)) - D(i-1)*cos(phi(i-1) - alpha_e(i-1)) - L(i-1)*sin(phi(i-1) - alpha_e(i-1)))/m;
    zdotdot(i) = ( -m*g + (T_top(i) + T_bot(i))*sin(phi(i-1)) + F_E(i)*cos(phi(i-1)) - D(i-1)*sin(phi(i-1) - alpha_e(i-1)) + L(i-1)*cos(phi(i-1) - alpha_e(i-1)))/m;
    phidotdot(i) = (M_air(i-1) + l_E*F_E(i) + l_P*(T_bot(i) - T_top(i)))/Iyy;
...
if animate == true
    h = figure();
    qbit_plot_trajectory(h, time,[x ; z ; phi], desired_state(1,:), desired_state(2,:),Fdes,l_P, save_animation)
    hold on
